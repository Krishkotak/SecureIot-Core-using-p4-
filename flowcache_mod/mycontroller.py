...
async def processPacket(message):
    ...
    Uses a lock to prevent race conditions from duplicate PacketIns.
    Returns the counter_index for logging.
    """
    ...
    if pktinfo['metadata']['punt_reason'] != \
            global_data['punt_reason_name2int']['FLOW_UNKNOWN']:
        reason = global_data['punt_reason_int2name'].get(
            pktinfo['metadata']['punt_reason'], 'UNKNOWN')
    ...
    for i in range(len(path)):
        ...
        if i == 0: # We only send PacketOut from the first switch
            packet_out_port = output_port

        task = addFlowRule(current_switch_obj,
                           src_ip_addr, dst_ip_addr, ip_proto,
                           output_port, new_dscp=5,
    ...
    if packet_out_port is not None:
        log.info(f"Sending PacketOut to {ingress_sw_name} to forward "
                 f"packet to port {packet_out_port}")
